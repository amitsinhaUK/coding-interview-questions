#
# Interview question: Given a complete binary tree, count the number of nodes
...
def count_nodes(binary_tree: BinaryTree, depth: int) -> int:
    current_node = binary_tree.root_node
    
    # Base case
    if current_node == None:
        return 0
    
    max_nodes_on_level = 2**(depth-1)
    max_nodes_on_level_index = max_nodes_on_level - 1
    binary_search_index_pointer_l = 0
    binary_search_index_pointer_r = max_nodes_on_level_index
    
    max_index_found = -1
    
    while binary_search_index_pointer_l <= binary_search_index_pointer_r:
        index_to_find = math.ceil((binary_search_index_pointer_r + binary_search_index_pointer_l) / 2)
        current_level = 1
        
        search_index_l = 0
        search_index_r = max_nodes_on_level_index
        
        while current_level < depth:
            if index_to_find < math.ceil((search_index_l+search_index_r)/2):
                current_node = current_node.left
                search_index_r = math.ceil((search_index_l+search_index_r)/2)
            else:
                current_node = current_node.right
                search_index_l = math.ceil((search_index_l+search_index_r)/2)

            current_level += 1
        
        if current_node != None:
            max_index_found = index_to_find
            binary_search_index_pointer_l = index_to_find + 1
        else:
            binary_search_index_pointer_r = index_to_find - 1
    
    max_nodes_above_level = (2**(depth-1))-1
        
    return max_nodes_above_level + (max_index_found + 1)
...
def main():
    ...
    tree_depth = find_depth(binary_tree=binary_tree)
    
    print(f"Tree depth is: {tree_depth}")
    print(f"Number of nodes in tree: {count_nodes(binary_tree=binary_tree, depth=tree_depth)}")
