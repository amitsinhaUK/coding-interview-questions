#
# Interview question: Given a binary tree, return the level order traversal of the nodes values as an arry
...
def breadth_first_search(binary_tree: BinaryTree) -> list:
    current_node = binary_tree.root_node
    parent_queue = deque()
    parent_queue.appendleft(current_node)
    node_levels = []
    ...
    while len(parent_queue) > 0:
        ...
        for node in parent_queue:
            if node != None:
                ...
                if node.left != None:
                    child_queue.appendleft(node.left)
                
                if node.right != None: 
                    child_queue.appendleft(node.right)

        node_levels.append(level_node_values)
...
def main():
    ...
    print(f"Level order of the Binary tree is: {breadth_first_search(binary_tree=binary_tree)}")
